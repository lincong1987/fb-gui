define(function (require, exports, module) {
	...
	DragEvent.create = function (elem, event, options) {
		...
		var classNameStart = elem.className
		                         .replace(/^\s|\s.*/g, '') + '-drag-resize-start';
		var classNameOver = elem.className
		                        .replace(/^\s|\s.*/g, '') + '-drag-resize-over';
		var classNameEnd = elem.className
		                       .replace(/^\s|\s.*/g, '') + '-drag-resize-end';
		// var minX;
		// var minY;
		// var maxX;
		// var maxY;
		...
		var options = options || {};
		// var rev_top = options.rev_top || 0;
		// var rev_left = options.rev_left || 0;
		//
		// $window = options.$window && $(options.$window) || $window;

		// var gridStep = options.gridStep || 5;
		// var scale = options.scale || 1;
		// var x = options.x || 1920;
		...
		dragEvent.onstart = function (event) {
			...
			var left = Math.round(event.clientX);
			var top = Math.round(event.clientY);
		...
		dragEvent.onover = function (event) {
			// var scale = api.appConfig.page.scale;
			//
			// var left = Math.round((event.clientX - api.offset.left + api.scroll.left) / scale);// - this.mouseStartLeft;
			// var top = Math.round((event.clientY - api.offset.top + api.scroll.top) / scale);// - this.mouseStartTop;
